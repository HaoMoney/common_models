...
class MMoE(object):
    ...
    def gate_kernel(self,inputs,input_dimension,num_experts,taskid):
        with tf.variable_scope("task_"+taskid):
            ...
            gate_layer = tf.nn.softmax(
                K.dot(x=inputs,y=weights) + biases)
            return gate_layer
    ...
    def build_model(self):
        ...
        with tf.variable_scope("gate_layer"):
            self.gate_output0 = self.gate_kernel(self.input_x2,self.input_dimension,self.num_experts,"0")
            self.gate_output1 = self.gate_kernel(self.input_x2,self.input_dimension,self.num_experts,"1")                
            #print(self.gate_outputs[0].shape)

        with tf.variable_scope('final_layer') as scope:
            expanded_gate_output0 = tf.expand_dims(self.gate_output0, axis=1)
            print(expanded_gate_output0.shape)
            repeated_tmp0 = tf.tile(expanded_gate_output0, [1, self.num_units, 1])
            #repeated_tmp0 = K.repeat_elements(expanded_gate_output0, self.num_units, axis=1)
            #print(repeated_tmp.shape)
            weighted_expert_output0 = self.expert_outputs2 * repeated_tmp0
            #print(weighted_expert_output.shape)
            self.final_output0 = tf.nn.l2_normalize(tf.reduce_sum(weighted_expert_output0, axis = 2), dim = 1)
            expanded_gate_output1 = tf.expand_dims(self.gate_output1, axis=1)
            #print(expanded_gate_output.shape)
            repeated_tmp1 = tf.tile(expanded_gate_output1, [1, self.num_units, 1])
            #repeated_tmp1 = K.repeat_elements(expanded_gate_output1, self.num_units, axis=1)
            #print(repeated_tmp.shape)
            weighted_expert_output1 = self.expert_outputs2 * repeated_tmp0
            #print(weighted_expert_output.shape)
            self.final_output1 = tf.nn.l2_normalize(tf.reduce_sum(weighted_expert_output1, axis = 2), dim = 1)
        ...
        with tf.variable_scope("sim"):
            self.sim1 = tf.reduce_sum(
				tf.multiply(self.final_output0, self.x1_output), axis=1)
            self.sim2 = tf.reduce_sum(
				tf.multiply(self.final_output1, self.x1_output), axis=1)
